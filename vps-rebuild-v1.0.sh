...
show_os_menu() {
    clear
    print_header "VPS REBUILD SCRIPT v1.0"
    echo ""
...
install_packages() {
    ...
    if [ "$PKG_MANAGER" = "apt" ]; then
        $PKG_INSTALL curl wget git vim nano htop net-tools \
            fail2ban unzip zip tar build-essential
    else
        $PKG_INSTALL curl wget git vim nano htop net-tools \
            fail2ban unzip zip tar epel-release
    fi
...
setup_memory() {
    ...
    echo "Pilih jenis memory:"
    echo "1. SWAP - Disk sebagai memory (lambat, stabil)"
    echo "2. ZRAM - Storage jadi RAM terkompresi (cepat, efisien)"
    echo "3. SWAP + ZRAM - Kombinasi keduanya (optimal)"
    echo "0. Skip"
    echo ""
...
setup_swap() {
    ...
    print_info "Membuat swap ${swap_size}GB..."
    ...
    print_success "Swap ${swap_size}GB dibuat!"
}
...
setup_zram() {
    ...
    print_info "ZRAM: Storage jadi RAM dengan kompresi 2-3x"
    echo ""
    
    if [ -d /sys/class/block/zram0 ]; then
        print_warning "ZRAM sudah aktif, skip..."
        return
    fi
    
    echo "Pilih ukuran ZRAM:"
    echo "1. 1 GB"
    echo "2. 2 GB"
    echo "3. 3 GB"
    echo "4. 4 GB"
    ...
    print_info "Setup ZRAM ${zram_size}GB..."
...
EOF
    ...
    print_success "ZRAM ${zram_size}GB aktif!"
}
...
main() {
    ...
    print_info "Script ini akan setup: Password, Packages, Timezone, Swap/Zram, Fail2Ban"
    echo ""
    ...
    clear
    print_header "MEMULAI REBUILD VPS v1.0"
    echo ""
    ...
    echo ""
    print_success "========================================="
    print_success "REBUILD VPS v1.0 SELESAI!"
    print_success "========================================="
    echo ""
